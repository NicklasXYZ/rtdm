...
const CustomDataTable = ({
  ...
  const handleChangeSelection = useCallback(
    (event) => {
      ...
      if (row) {
        ...
        updateRowState(row.dataset.rowId, currentTarget.checked)
      }
    },
    [setRowSelection, updateRowState]
  );

  const handleChangeSelectionAll = useCallback(
    (event) => {
      setRowSelection(undefined, event.currentTarget.checked);
      updateAllRowState(event.currentTarget.checked)
    },
    [setRowSelection]
  );
  ...
  return (
    <TableContainer title="DataTable" description="">
      <TableToolbar>
        {/* <TableBatchActions
